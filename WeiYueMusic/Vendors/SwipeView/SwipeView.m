...
{
    if (ABS(_decelerationRate - decelerationRate) > 0.0001f)
    {
...
{
    if (ABS(_autoscroll - autoscroll) > 0.0001f)
    {
...
- (void)updateScrollOffset
{
    ...
    if (_vertical && ABS(_scrollView.contentOffset.x) > 0.0001f)
    {
        [self setContentOffsetWithoutEvent:CGPointMake(0.0f, _scrollView.contentOffset.y)];
    }
    else if (!_vertical && ABS(_scrollView.contentOffset.y) > 0.0001f)
    {
...
- (void)didScroll
{
    ...
    if (!_defersItemViewLoading || ABS([self minScrollDistanceFromOffset:_lastUpdateOffset toOffset:_scrollOffset]) >= 1.0f)
    {
        //update item index
...
{
    if (ABS(_scrollOffset - scrollOffset) > 0.0001f)
    {
...
- (void)scrollViewDidEndDecelerating:(__unused UIScrollView *)scrollView
{
    ...
    if (ABS(_scrollOffset - integerOffset) < 0.01f)
    {
